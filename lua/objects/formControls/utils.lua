function generateControlSelf(_type, _name, _position, _size, _form, _options)
  ...
  if _type == "TextControl" then
    ...
    s.object = GameObject(_position, Ivec2(s.label_size + s.value_size, 1), _form.game_scene or defaultGameScene)
    s.colours.default_text = colours.default_text or characters.DEFAULT_FOREGROUND_COLOUR
    s.colours.default_bg = colours.default_bg or characters.DEFAULT_BACKGROUND_COLOUR
    s.colours.hover_text = colours.hover_text or characters.DEFAULT_FOREGROUND_COLOUR
    s.colours.hover_bg = colours.hover_bg or Colours.DARK_GREY
    ...
    s.colours.error_text = colours.error_text or characters.DEFAULT_FOREGROUND_COLOUR
    s.colours.error_bg = colours.error_bg or Colours.RED
  ...
  elseif _type == "CheckboxControl" then
    ...
    for _, v in ipairs(options) do
      ...
      label_size = math.max(label_size, utf8.len(v.label))
    end

    s.size = Ivec2(label_size + 1, #options)

    for k, v in ipairs(options) do
      table.insert(s.options.objects, GameObject(_position + Ivec2(0, k - 1), Ivec2(label_size + 1, 1), _form.game_scene or defaultGameScene))
    end
    ...
    s.colours.unchecked_text = colours.unchecked_text or characters.DEFAULT_FOREGROUND_COLOUR
    s.colours.default_bg = colours.default_bg or characters.DEFAULT_BACKGROUND_COLOUR
    s.colours.selected_bg = colours.selected_bg or Colours.DARK_GREY
    s.colours.error_bg = colours.error_bg or Colours.RED
  end
