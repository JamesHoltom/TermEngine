...
namespace term_engine::rendering {
  GameWindow::GameWindow() :
    GameWindow(DEFAULT_POSITION, DEFAULT_SIZE, std::string(DEFAULT_TITLE), 0)
  {}

  GameWindow::GameWindow(const glm::ivec2& position, const glm::ivec2& size, const std::string& title, uint32_t flags) :
    window_(nullptr),
  ...
  void GameWindow::SetResizable(bool flag) const
  {
  ...
  void GameWindow::SetMouseGrab(bool flag) const
  {
    SDL_SetWindowMouseGrab(window_, flag ? SDL_TRUE : SDL_FALSE);
  }

  void GameWindow::SetWireframe(bool flag)
  {
  ...
  void GameWindow::SetVsync(int flag)
  {
    if (flag >= -1 && flag <= 1)
    {
